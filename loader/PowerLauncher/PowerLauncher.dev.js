...
var sp=cp+"/source"
var rp=cp+"/release"
var pp=lp+"/PowerLauncher"
var dp=pp+"/dist"
function main(){
    var manifsrc=F.read(cp+"/package.json")
    var manifest=JSON.parse(manifsrc);
    var relcode=F.read(pp+"/PowerLauncher.src.js");
    var anames=new Array()
    var assets=new Array()
    var snames=new Array()
    var srcjss=new Array()
    var stack=0
    
    var aparser=(pathv)=>{
        var file=new _File(pathv)
        var pathlist=file.list()
        var filelist=Lpath2Lfile(pathv,pathlist)
        for(var i=0;i<filelist.length;i++){
            if(filelist[i].isDirectory()){
                aparser(filelist[i].getPath());
            }else{
                anames.push(""+filelist[i].getPath().replace(ap+"/",""))
                //print(anames)
                assets.push(""+D.b64e(F.readbin(filelist[i].getPath())))
            }
        }
    }
    aparser(ap);
    ...
    var jparser=(patht)=>{
        ...
        for(var i=0;i<filelist.length;i++){
            if(filelist[i].isDirectory()){
                ...
                //print(anames)
                srcjss.push(""+F.read(filelist[i].getPath())+" ")
            }
        }
    }
    jparser(sp);
    FILENAME=anames.toSource();
    DATA=assets.toSource();
    CODENAME=snames.toSource();
    CODE=srcjss.toSource();
    var output="";
    
    output=relcode.replace("{{FILENAME}}",FILENAME)
    output=output.replace("{{DATA}}",DATA)
    output=output.replace("{{CODENAME}}",CODENAME)
